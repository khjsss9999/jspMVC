...
<%--
	...
	2. 다운받은 4개의 파일을 WEB-INF 안에 lib 폴더에 넣어주자. (.jar 파일을 넣었다.)
	3. JSTL을 사용하기 위해서는 지시어 중 taglib 선언하고 사용한다.(보통은 page 지시어 밑)
 ...
 --%>
	...
	<%-- 변수 삭제(remove) --%>
	<c:remove var="str"/>
	<c:out value="${str }" /> <br> <%-- 변수값 출력해도 삭제해서 안 나옴 --%>
	<hr>
	
	<%-- 
		if 문 : else 가 없다. 조건식이 참일때만 실행
		<c:if test="조건식("관계연산,비교연산,논리연산 -> EL 방식으로 작성" var="변수")"></c:if>
	 --%>
	...
	<%--
		...
		2. 개선된 for문
		<c:forEach var="사용할 변수" items="배열이나ArrayList를 EL형식으로 표현"></c:forEach>
